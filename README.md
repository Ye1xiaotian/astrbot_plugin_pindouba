...
命令只有一个：引用图片消息并发送`/拼豆`。私聊群聊都一样，不需要机器人的唤醒前缀。
...
两次使用之间有 30 秒冷却（可在插件配置界面调整时长，放置刷屏）。
...
- **拼豆色卡**：打开 `bead_palette` 后，颜色只从 56 色经典拼豆色卡里选，更有手工感，默认关闭（测试下来若原图色彩过于丰富，会与原图色差较大，一般不推荐开启）
